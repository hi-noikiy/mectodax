...
class FW_Shortcode_Map2 extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => esc_html__('Custom','campaign'),
					'options'    => array(
						'locations' => array(
							'label' => esc_html__('Locations', 'campaign'),
							'popup-title' => esc_html__('Add/Edit Location', 'campaign'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . esc_html__('Note: Please set location', 'campaign') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' => esc_html__('Location','campaign'),
								),
								'title' => array(
									'type' => 'text',
									'label' => esc_html__('Location Title', 'campaign'),
									'desc' => esc_html__('Set location title', 'campaign'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => esc_html__('Location Description', 'campaign'),
									'desc'  => esc_html__('Set location description', 'campaign')
								),
								'url' => array(
									'type'  => 'text',
									'label' => esc_html__('Location Url', 'campaign'),
									'desc'  => esc_html__('Set page url (Ex: http://example.com)', 'campaign'),
								),
								'thumb' => array(
									'label'       => esc_html__('Description Image', 'campaign'),
									'desc'        => esc_html__('Add location image (100x100)', 'campaign'),
									'type'        => 'upload',
								),
								'pin' => array(
									'label'       => esc_html__('Location Pin', 'campaign'),
									'type'        => 'upload',
								)
							)
						)
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				esc_html__('No location provider specified for map shortcode', 'campaign')
			);
			return '<b>' . esc_html__( 'Map Placeholder', 'campaign' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			trigger_error(
				sprintf(esc_html__('Unknown location provider "%s" specified for map shortcode', 'campaign'), $provider)
			);
			return '<b>' . esc_html__( 'Map Placeholder', 'campaign' ) . '</b>';
		}
